var GAME = {
  init: function () {
    this.players = [];
    this.players[0] = new AI("the AI", true);
    ...
    this.intersection = [
      1, 9, 17, 3, 11, 19, 5, 13, 21, 7, 15, 23, 0, 8, 16, 2, 10, 18, 4, 12, 20,
      6, 14, 22,
    ];
  ...
  getCurrentPhase: function (player) {
    ...
    if (isPlacingPhase) return PHASE.PLACING;

    var isMovingPhase = player.phase.value === PHASE.MOVING.value;
    if (isMovingPhase) return PHASE.MOVING;

    var isFlyingPhase = player.phase.value === PHASE.FLYING.value;
    if (isFlyingPhase) return PHASE.FLYING;
  },
  ...
  checkTie: function () {
    ...
    var isBothFlyingPhase =
      this.currentPlayer.phase === PHASE.FLYING &&
    ...
    if (this.catchCountdown === 0) {
      ...
      var placingPhaseIsOver =
        this.currentPlayer.stockPieces === 0 &&
  ...
  checkEnemyFail: function () {
    ...
    var hasLost =
      this.enemyHasLessThanThreePieces() ||
  ...
  isEnemySurrounded: function () {
        ...
        if (enemy === marker) {
          _.each(
            ...
            function (connection) {
              ...
              var neighborAvailable =
                this.board[_.without(connection, index)[0]] === undefined;
    ...
    var enemyNoPiecesInStock =
      this.getEnemy(this.currentPlayer).stockPieces === 0;
  ...
  isValidPosition: function (position, hasToBeEmptyPosition) {
    var isBadPosition =
      position === undefined || position < 0 || position > this.boardSize - 1;
